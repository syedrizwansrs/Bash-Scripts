...
install_proxmox() {
    ...
    # Remove UEFI options completely
    echo -e "Booting in default mode."
    ...
    qemu-system-x86_64 \
        ...
        -drive file=/dev/sdb,format=raw,media=disk,if=virtio -vnc :0 -serial stdio
}
...
boot_proxmox_with_port_forwarding() {
    ...
    # Remove UEFI options completely
    echo -e "${CLR_YELLOW}Booting in default mode.${CLR_RESET}"
    ...
    nohup qemu-system-x86_64 -enable-kvm \
        ...
        -smp 4 -m 4096 \
        -drive file=/dev/sda,format=raw,media=disk,if=virtio \
        -drive file=/dev/sdb,format=raw,media=disk,if=virtio -vnc :0 \
        > qemu_output.log 2>&1 &
